...
type TaskEditor interface {
	...
	Add(name string, due string) (int16, error)                // Create
	Get() []Task                                               // Read
...
func (tm *TaskManager) Add(name string, due string) (int16, error) {
	if strings.TrimSpace(name) == "" {
		return 0, errors.New("Task name must not be empty")
	}
	if strings.TrimSpace(due) == "" {
		return 0, errors.New("Task due date must not be empty")
	}
